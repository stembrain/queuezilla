...
$conn = new XMPPHP_XMPP('talk.google.com', 5222, 'aceinfo06', 'Trh2doors.', 'xmpphp', 'gmail.com', $printlog=true, $loglevel=XMPPHP_Log::LEVEL_INFO);
$conn->autoSubscribe();
    	...
    	foreach($payloads as $event) {
    		...
    		switch($event[0]) {
    			case 'message':
    				...
    				$matches = array();
    				if(preg_match('/queue\s+(.+)/', $pl['body'], $matches)){
    					$searchTerm = urlencode($matches[1]);
    					list($jabberAccount,) = explode('/', $pl['from']);
    					 
    					$titleAdded = file_get_contents('http://www.stembrain.com/lab/queues/searchAndAddTitle/' 
    						. $searchTerm . '/' . base64_encode($jabberAccount));
    						
    					if(strlen($titleAdded)){
		    				$conn->message($pl['from'], $body="Added $titleAdded to your netflix streaming queue at position 0.", $type=$pl['type']);
    					}
    					else{
							$conn->message($pl['from'], $body="The title {$matches[1]} could not be added to your netflix streaming queue.", $type=$pl['type']);
    					}
    				}
    				else{
